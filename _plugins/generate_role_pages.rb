...
#      )
      ...
      # Generate a page for each role
      framework['roles'].each do |role|
        site.collections['roles'].docs << create_doc(
          site, '_roles',
         "#{role['id']}.md",
          {
            'title' => role['title'],
      ...
      framework['roles'].each do |role|
        counts = summarize_tasks_by_practice_and_role(role)
      end
    ...
    def summarize_tasks_by_practice_and_role(data)
      practice_map = {}
              ...
              if ref.is_a?(Hash)
                ...
                if source
                  if source.is_a?(String)
                    roles.add(source) unless source.empty?
                  else
                    roles.add(source)
                  end
                end
